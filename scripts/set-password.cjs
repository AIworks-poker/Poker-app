...
/**
 * The admin address is deployment configuration, not source. It used to be a
 * literal here, which published a personal address in what is a PUBLIC repo for
 * no functional gain — the app itself has always read ADMIN_EMAIL at runtime.
...
;(async () => {
  ...
  for (const [name, env] of [['PREVIEW', keys.preview], ['PRODUCTION', keys.production]]) {
    ...
    await update(name, url, hash, admin)
  }
